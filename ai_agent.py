...
def chat_with_ai(user_input):
    ...
    # Check if any valid context is returned
    if context_chunks and any(chunk.strip() for chunk in context_chunks):
        context = "\n---\n".join(context_chunks)
        prompt = (
            f"Use the following context to answer the question. If the context lacks sufficient detail, supplement your answer using your own knowledge.\n\n"
            f"Context:\n{context}\n\n"
            f"Question: {user_input}\n\n"
            f"Answer:"
        )
    else:
        # No relevant context found; rely entirely on LLM's general knowledge.
        prompt = (
            f"Answer the following question using your general knowledge. Provide a concise and accurate answer.\n\n"
            f"Question: {user_input}\n\n"
            f"Answer:"
        )
